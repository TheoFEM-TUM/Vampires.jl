...
@testset "convergence ENCUT" begin
    ...
    for (folder, value) in zip(keyword * "_" .* values, values)
        ...
        @test find_value(incar_, keyword) == value
...
@testset "NSCF" begin
    ...
    @test find_value(scf_incar, "ISTART") == "0"
    @test find_value(scf_incar, "LCHARG") == "True"

    nscf_incar = read_incar(path*"nscf/INCAR")
    @test find_value(nscf_incar, "ICHARG") == "11"
    @test find_value(nscf_incar, "LCHARG") == "False"
...
@testset "StrongScaling" begin
    ...
    for keyword in ["cpu", "gpu"]
        ...
        for (i, kpar, ncore_nsim) in zip(collect(1:length(kpar_range)), kpar_range, ncore_nsim_range)
            ...
            @test find_value(incar, "KPAR") == string(kpar)

            if keyword == "cpu"
                @test find_value(incar, "NCORE") == string(ncore_nsim)
            elseif keyword == "gpu"
                @test find_value(incar, "NSIM") == string(ncore_nsim)
            end

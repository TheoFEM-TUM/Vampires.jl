...
@testset "SLURM Script" begin
    script_path = test_file_path * "test_jobscript.job"
    Vampires.write_slurm_script("vasp_std", test_file_path; module_paths=["/path/to/modules"], module_list=["module1", "module2"],
                       time=2, nodes=2, ntasks_per_node=96, ntasks_per_core=2, omp_num_threads=12, num_gpu=4, partition="batch",
                       mail="user@example.com", filename="test_jobscript")

    script_content = read_slurm_script(script_path)

    # Test SBATCH directives
    @test extract_sbatch_directive(script_content, "nodes") == "2"
    @test extract_sbatch_directive(script_content, "ntasks-per-node") == "4"
    @test !(extract_sbatch_directive(script_content, "ntasks-per-node") == "96")
    ...
    Vampires.write_slurm_script("vasp_std", test_file_path; module_paths=["/path/to/modules"], module_list=["module1", "module2"],
    time=2, nodes=2, ntasks_per_node=96, ntasks_per_core=2, omp_num_threads=0, num_gpu=4, partition="batch",
    mail="user@example.com", filename="test_jobscript")

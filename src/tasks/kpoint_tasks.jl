...
# Arguments
- `par`: Specifies the parameter for generating the k-grid. If set to `KSPACING`, the function will use the provided `val` to determine the k-spacing.
- `val`: The value corresponding to the `par` parameter. If `par` is `KSPACING`, this value is used to set the k-spacing for the k-grid.
- `N`: The number of k-points in each direction, specified as a comma-separated list (e.g., `10,10,10`). Used if `par` is not `KSPACING`.
- `poscar`: Path to the POSCAR file used to determine the lattice parameters for k-point generation (optional; default is "POSCAR").
- `method`: Defines the method to generate the k-point grid. Default is Gamma-centered, if first letter of method is "m", Monkhorst-Pack grid will be used instead.
- `o`: Name of the output file where the KPOINTS data will be written. The default filename is "KPOINTS".
